...
def _flat_ground(
    z: float = 0.0, half_extent: float = 50.0
...
def _sloped_ground(
    pitch_deg: float, half_extent: float = 50.0
) -> tuple[np.ndarray, np.ndarray]:
...
def test_ground_snapper_rejects_bad_shapes() -> None:
    with pytest.raises(ValueError):
        GroundSnapper(
            np.zeros((3, 4), dtype=np.float32), np.zeros((1, 3), dtype=np.int32)
        )
    with pytest.raises(ValueError):
        GroundSnapper(
            np.zeros((3, 3), dtype=np.float32), np.zeros((1, 4), dtype=np.int32)
        )
