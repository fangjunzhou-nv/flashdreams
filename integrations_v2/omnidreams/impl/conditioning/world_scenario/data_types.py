...
class PolylineElement(MapElement):
    ...
    def __post_init__(self) -> None:
        ...
        if self.points.size > 0 and (
            self.points.ndim != 2 or self.points.shape[1] != 3
        ):
            raise ValueError(
                f"Polyline points must have shape (N, 3), got {self.points.shape}"
            )
...
class PolygonElement(MapElement):
    ...
    def __post_init__(self) -> None:
        ...
        if self.vertices.size > 0 and (
            self.vertices.ndim != 2 or self.vertices.shape[1] != 3
        ):
            raise ValueError(
                f"Polygon vertices must have shape (N, 3), got {self.vertices.shape}"
            )
...
class OrientedBoxElement(MapElement):
    ...
    def __post_init__(self) -> None:
        ...
        if self.orientation.shape != (4,):
            raise ValueError(
                f"Orientation must have shape (4,), got {self.orientation.shape}"
            )
...
class LaneLine(PolylineElement):
    ...
    lane_type: LaneLineType = field(
        default_factory=lambda: LaneLineType(
            color=LaneLineColor.UNKNOWN, style=LaneLineStyle.UNKNOWN
        )
    )
...
class Pole(PolylineElement):
    ...
    @classmethod
    def from_base_point(
        cls, element_id: str, base: NDArray[np.float32], height: float = 3.0
    ) -> "Pole":
...
class TrafficLight(OrientedBoxElement):
    ...
    states: Dict[int, TrafficLightState] = field(
...
class DynamicObject:
    ...
    def __post_init__(self) -> None:
        ...
        if self.centers.shape[0] != n_frames or self.centers.shape[1] != 3:
            raise ValueError(
                f"Centers shape mismatch: expected ({n_frames}, 3), got {self.centers.shape}"
            )

        if self.orientations.shape[0] != n_frames or self.orientations.shape[1] != 4:
            raise ValueError(
                f"Orientations shape mismatch: expected ({n_frames}, 4), got {self.orientations.shape}"
            )
        ...
        if self.dimensions.ndim == 1:
            if self.dimensions.shape[0] != 3:
                raise ValueError(
                    f"Dimensions must be (3,) or (N, 3), got {self.dimensions.shape}"
                )
            # Broadcast to all frames
            self.dimensions = np.tile(self.dimensions, (n_frames, 1))
        elif self.dimensions.shape != (n_frames, 3):
            raise ValueError(
                f"Dimensions shape mismatch: expected ({n_frames}, 3), got {self.dimensions.shape}"
            )
    ...
    def _extrapolate_before(
        ...
        if dt_keyframes <= 0:
            return (
                self.centers[0].copy(),
                self.dimensions[0].copy(),
                self.orientations[0].copy(),
            )
    ...
    def _extrapolate_after(
        ...
        if dt_keyframes <= 0:
            return (
                self.centers[-1].copy(),
                self.dimensions[-1].copy(),
                self.orientations[-1].copy(),
            )
