...
from omnidreams.impl.conditioning.world_scenario.camera_base import CameraBase
...
def compute_max_radius(
    ...
    principal_point: np.ndarray | tuple[float, float],
) -> float:
...
def compute_ftheta_fw_mapping_max_angle(
    fwpoly: np.ndarray, max_radius_pixels: float
) -> float:
    """
    ...
    for i in range(1, DOMAIN_SAMPLE_COUNT + 1):
        ...
        d_next_r = eval_polynomial_derivative(
            np.asarray(next_angle).reshape(1), fwpoly
        ).item()
...
def approximate_polynomial_inverse(
    coeffs: np.ndarray, range_min: float, range_max: float
) -> np.ndarray:
    """
    ...
    def f5(
        x: float, b: float, x1: float, x2: float, x3: float, x4: float, x5: float
    ) -> float:
    ...
    samples_x = eval_polynomial(
        samples_y, coeffs.astype(np.float64)
    )  # use high-precision estimation
    ...
    return np.array(
        [np.float32(val) if i > 0 else 0 for i, val in enumerate(inv_coeffs)],
        dtype=np.float32,
    )
...
class FThetaCamera(CameraBase):
    ...
    def from_dict(cls, rig_dict: Dict[str, Any]) -> "FThetaCamera":
        ...
        Returns:
            FThetaCamera: the newly created object.
        """
        cx, cy, width, height, bw_poly, is_bw_poly, linear_cde = (
    ...
    @classmethod
    def from_numpy(
        cls, intrinsics: np.ndarray, device: Optional[str] = None
    ) -> "FThetaCamera":
        ...
        return cls(
            ...
            if len(intrinsics) < 14
            else intrinsics[11:14],
            device=device,
        )
    ...
    def __init__(
        ...
        assert len(linear_cde) == 3, (
            f"linear_cde must have 3 coefficients, got {len(linear_cde)}"
        )
        ...
        self._intrinsics = np.array(
            [cx, cy, width, height, *poly, 1 if is_bw_poly else 0, *linear_cde],
            dtype=np.float32,
        )
    ...
    def _cache_torch_tensors(self) -> None:
        # caches in torch
        self._max_ray_angle_torch = torch.tensor(
            self._max_ray_angle, dtype=self.dtype, device=self.device
        )
        self._fw_poly_torch = torch.tensor(
            self._fw_poly.coef, dtype=self.dtype, device=self.device
        )
        self._fw_poly_powers_torch = torch.arange(
            len(self._fw_poly_torch), dtype=self.dtype, device=self.device
        )
        self._max_ray_distortion_torch = torch.tensor(
            self._max_ray_distortion, dtype=self.dtype, device=self.device
        )
        self._center_torch = torch.tensor(
            self._center, dtype=self.dtype, device=self.device
        )
        self._A_torch = torch.tensor(self.A, dtype=self.dtype, device=self.device)
        self._inv_A_torch = torch.tensor(
            self.inv_A, dtype=self.dtype, device=self.device
        )
    ...
    def rescale(
        ...
        if ratio is not None:
            ...
            raise ValueError(
                "Must provide either 'ratio' or at least one of 'ratio_h'/'ratio_w'"
            )
    ...
    def get_ftheta_parameters_from_json(rig_dict: Dict[str, Any]) -> tuple[Any, ...]:
        ...
        if "bw-poly" in props:  # Is this a regular rig?
            ...
            if poly_type == "pixeldistance-to-angle":
                ...
                raise ValueError(
                    f"Unsupported polynomial type: {poly_type} from rig dict {rig_dict}"
                )
    ...
    def _compute_bw_poly(self) -> Polynomial:
        ...
        max_value = compute_ftheta_fw_mapping_max_angle(
            self._fw_poly.coef, max_pixel_radius
        )
    ...
    def ray2pixel_torch(self, rays: torch.Tensor) -> torch.Tensor:
        ...
        delta[alpha_cond] = torch.sum(
            self._fw_poly_torch * alpha_powers, dim=-1, keepdim=False
        )

        # For outside the model (which need to do linear extrapolation)
        delta[~alpha_cond] = (
            self._max_ray_distortion_torch[0]
            + (alpha[~alpha_cond] - self._max_ray_angle_torch)
            * self._max_ray_distortion_torch[1]
        )
        ...
        result = (
            torch.einsum("ij,nj -> ni", self._A_torch, pixel[:, :2])
            + self._center_torch
        )
