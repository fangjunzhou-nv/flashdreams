...
from omnidreams.impl.conditioning.world_scenario.data_loaders import (
    SceneDataLoader,
    auto_register,
)
...
from omnidreams.impl.conditioning.world_scenario.data_utils import (
...
from omnidreams.impl.conditioning.world_scenario.laneline_utils import (
    build_lane_line_type,
)
...
class ClipGTLoader(SceneDataLoader):
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        if not isinstance(source, (str, Path)):
            raise TypeError(
                f"ClipGTLoader only supports string or Path sources, got {type(source)!r}"
            )
        ...
        files = {
            ...
            "camera_timestamps": clipgt_path
            / f"{clip_id}.camera_front_wide_120fov.json",
        }
        ...
        if files["egomotion"].exists():
            self._load_ego_poses(
                ...
                camera_timestamps_file=files["camera_timestamps"]
                if files["camera_timestamps"].exists()
                else None,
            )
    ...
    def _load_ego_poses(
        ...
        if camera_timestamps is not None:
            ...
            duration = (
                target_timestamps_micros[-1] - target_timestamps_micros[0]
            ) / 1e6
            ...
            target_timestamps_seconds = np.linspace(
                0, (num_frames - 1) / target_fps, num_frames
            )
    ...
    def _load_camera_timestamps(
        self, camera_timestamps_file: Optional[Path]
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            timestamps = np.array(
                [frame["timestamp"] for frame in cam_data], dtype=np.int64
            )
            logger.info(
            ...
            logger.warning(
                f"Failed to load camera timestamps from {camera_timestamps_file}: {e}"
            )
            return None
    ...
    def _load_camera_calibrations(
        ...
        if camera_names is None:
            camera_names = [
                s["name"].replace(":", "_")
                for s in sensors
                if s.get("name", "").startswith("camera:")
            ]
        ...
        for camera_name in camera_names:
            ...
            poly_coeffs = np.array(
                [float(x) for x in poly_str.split()], dtype=np.float32
            )
                ...
                if len(colors) > 0 and len(styles) > 0:
                    ...
                    if color_str:
                        try:
                            ...
                            logger.debug(
                                f"Unknown lane line color: {color_str}, using UNKNOWN"
                            )
                    if style_str:
                        try:
                            ...
                            logger.debug(
                                f"Unknown lane line style: {style_str}, using UNKNOWN"
                            )
    ...
    def _load_road_boundaries(
        self, scene_data: SceneData, road_boundary_file: Path
    ) -> None:
        ...
        for idx, row in df.iterrows():
            ...
            if "location" in pole:
                ...
                if len(loc) >= 2:
                    ...
                    base = np.array(
                        [loc[0]["x"], loc[0]["y"], loc[0]["z"]], dtype=np.float32
                    )
                    base_rdf = convert_points_flu_to_rdf(base.reshape(1, 3))[0]
                    scene_data.poles.append(
                        Pole.from_base_point(f"pole_{idx}", base_rdf, height=3.0)
                    )
                    continue
                else:
                    continue
    ...
    def _load_road_markings(
        self, scene_data: SceneData, road_marking_file: Path
    ) -> None:
        """Load road markings."""
    ...
    def _load_traffic_lights(
        self, scene_data: SceneData, traffic_light_file: Path
    ) -> None:
        """Load traffic lights."""
        ...
        for idx, row in df.iterrows():
            ...
            if "dimensions" in light:
                ...
                if all(dims[k] is not None for k in ["x", "y", "z"]):
                    dimensions = np.array(
                        [dims["x"], dims["y"], dims["z"]], dtype=np.float32
                    )
    ...
    def _load_traffic_signs(
        self, scene_data: SceneData, traffic_sign_file: Path
    ) -> None:
        """Load traffic signs."""
    ...
    def _load_intersection_areas(
        self, scene_data: SceneData, intersection_file: Path
    ) -> None:
