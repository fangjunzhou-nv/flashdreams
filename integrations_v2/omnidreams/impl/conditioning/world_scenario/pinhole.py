...
def intrinsic_params_to_matrices(params: torch.Tensor) -> torch.Tensor:
    """Convert (fx, fy, cx, cy) parameters to camera intrinsic matrix/matrices.

    Args:
        params (torch.Tensor [...,4]): Intrinsic parameters (fx, fy, cx, cy).

    Returns:
        K (torch.Tensor [...,3,3]): Camera intrinsic matrices.
    """
    assert params.shape[-1] == 4, (
        "Intrinsic params must have shape (..., 4) for (fx, fy, cx, cy)."
    )
...
class PinholeCamera(CameraBase):
    ...
    def rescale(self, ratio_h: float, ratio_w: Optional[float] = None) -> None:
        ...
        self._intrinsics = np.array(
            [self.fx, self.fy, self.cx, self.cy, self.w, self.h], dtype=np.float32
        )
        self.cache_torch_and_np_intrinsics()
    ...
    def get_intrinsics_matrix(self) -> torch.Tensor:
        ...
        params = torch.tensor(
            [self.fx, self.fy, self.cx, self.cy], device=self.device, dtype=self.dtype
        )
        K = intrinsic_params_to_matrices(params)
        assert isinstance(K, torch.Tensor), (
            "Expected torch.Tensor from Camera helper when given torch input"
        )
        return K
