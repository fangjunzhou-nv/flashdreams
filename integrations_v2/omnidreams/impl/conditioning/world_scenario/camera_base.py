...
import numpy as np
import torch
from omnidreams.impl.conditioning.world_scenario.pcd_utils import (
...
class CameraBase:
    ...
    def get_rays_posed(
        self, camera_poses: torch.Tensor
    ...
    def draw_line_depth(
        ...
        for camera_to_world in camera_poses:
            ...
            for p in polylines:
                ...
                if (
                    self.transform_points_np(polyline, world_to_camera)[:, 2] < 0
                ).all():
                    continue
                ...
                v_round = np.round(uv_coords[:, 1]).astype(np.int32)
                valid_uv_mask = (
                    (u_round >= 0) & (u_round < W) & (v_round >= 0) & (v_round < H)
                )
