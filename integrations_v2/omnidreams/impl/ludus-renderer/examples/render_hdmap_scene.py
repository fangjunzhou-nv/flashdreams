...
def render_single_frame(args):
    ...
    print(
        f"Rendering frame {args.frame} (timestamp {timestamps[args.frame].item()}) with {camera_mode}"
    )
...
def render_sequence(args, all_cameras: bool = False):
    ...
    for cam_name in camera_names:
        ...
        poses, _ = compute_camera_poses(
            scene, timestamps, device, bev_height=bev_h, camera_name=cam_name
        )
...
def render_overlay_sequence(args):
    ...
    print("  Computing camera poses...")
    poses, _ = compute_camera_poses(
        scene, timestamps, device, bev_height=bev_height_val, camera_name=camera_name
    )
