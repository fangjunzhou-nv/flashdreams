...
class OrthographicCamera:
    ...
    """Optical center in pixels with shape ``[2]``."""

    image_size: torch.Tensor
    """Image width and height in pixels with shape ``[2]``."""

    pixels_per_meter: torch.Tensor
    """Horizontal and vertical projection scale with shape ``[2]``."""

    depth_max: float = 100.0
    """Maximum camera-space depth used for the z-buffer."""
...
def _pack_cameras(
    ...
    for cam in cameras:
        if isinstance(cam, OrthographicCamera):
            scale = cam.pixels_per_meter.cpu().flatten()[:2]
            row = torch.cat(
                [
