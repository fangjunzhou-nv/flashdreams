...
if TYPE_CHECKING:
    from ludus_renderer.torch import OrthographicCamera
...
def create_bev_camera(
    ...
    """Create a straight-down orthographic bird's-eye-view camera.

    ``bev_height`` and ``fov_deg`` retain their familiar footprint controls:
    they define the ground height that a perspective camera would cover, then
    convert that footprint to a constant orthographic scale.

    Raises:
        ValueError: A dimension, height, or field of view is invalid.
    """
    from ludus_renderer.torch import OrthographicCamera
    ...
    if bev_height <= 0.0:
        raise ValueError("BEV height must be positive.")
    if not 0.0 < fov_deg < 180.0:
        raise ValueError("BEV field of view must be between 0 and 180 degrees.")

    cx, cy = width / 2.0, height / 2.0
    ground_height = 2.0 * bev_height * math.tan(math.radians(fov_deg) / 2.0)
    pixels_per_meter = height / ground_height

    return OrthographicCamera(
        principal_point=torch.tensor([cx, cy], device=device),
        image_size=torch.tensor([float(width), float(height)], device=device),
        pixels_per_meter=torch.tensor(
            [pixels_per_meter, pixels_per_meter], device=device
        ),
        depth_max=far,
    )
