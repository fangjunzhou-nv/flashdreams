...
// Packed camera intrinsics (72 bytes = 18 floats). F-theta cameras map
// incident angle θ to pixel radius r via r = Σ(poly[i] * θ^i).
// Orthographic cameras set max_ray_angle < 0 and store pixels-per-metre in
// fw_poly[0:2].
struct FThetaCamera {
