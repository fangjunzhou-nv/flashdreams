...
"""One-shot setup helper for every ``flashdreams-omnidreams`` application.
...
def scene_path(scene_uuid: str, variant: str = SCENE_VARIANT_DEFAULT) -> Path:
    ...
    see :func:`omnidreams.impl.scenes.local_scene_archive_path` for the exact
...
def _sync_thirdparty_module() -> ModuleType:
    ...
    It lives in the ``omnidreams_singleview`` source tree under ``impl/`` and
    is not an installed module, so it's loaded from its file rather than
    imported.
    """
    tool_path = (
