...
_VARIANT_PROMPT_INDEX = {_DEFAULT_VARIANT: 1, "snow": 2, "rain": 3}


def prompt_variant_for_scene_variant(variant: str) -> str:
    """Map a scene variant slug to its in-archive prompt key."""
    slug = (variant or _DEFAULT_VARIANT).strip()
    return slug if slug.isdecimal() else str(_VARIANT_PROMPT_INDEX.get(slug, 1))


def _variant_suffix(variant: str) -> str:
    slug = (variant or _DEFAULT_VARIANT).strip()
    return "" if slug in {"", _DEFAULT_VARIANT} else f"-{slug}"


def resolve_variant_archive(scene_path: Path, variant: str) -> Path:
    """Resolve a weather-variant archive next to the selected scene."""
    scene_path = Path(scene_path)
    stem = scene_path.stem.removeprefix("clipgt-")
    uuid = stem[:36] if len(stem) >= 36 else stem
    candidate = scene_path.with_name(f"clipgt-{uuid}{_variant_suffix(variant)}.usdz")
    return candidate if candidate != scene_path and candidate.exists() else scene_path


def variant_from_stem(stem: str, prefix: str) -> str | None:
    """Extract a default, underscored, or numeric variant from a file stem."""
    if stem == prefix:
...
def _load_ground_mesh(
    ...
    :class:`interactive_drive.simulation.ground_snap.GroundSnapper`, or ``(None, None)`` when the
