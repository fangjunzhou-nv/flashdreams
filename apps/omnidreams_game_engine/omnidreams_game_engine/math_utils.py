...
def sample_polyline(points: object, spacing_m: float) -> object:
    ...
    target_distances = np.arange(
        0.0, total_length + spacing_m * 0.5, spacing_m, dtype=np.float32
    )
    sampled = []
    seg_index = 0
    for target in target_distances:
        while seg_index + 1 < len(distances) and distances[seg_index + 1] < float(
            target
        ):
            seg_index += 1
        ...
        alpha = (
            0.0 if end_d <= start_d else (float(target) - start_d) / (end_d - start_d)
        )
...
def offset_segments(segments: object, offset_m: float) -> object:
    ...
    for p0, p1 in segs:
        ...
        offsets.append(
            np.array(
                [normal_xy[0] * offset_m, normal_xy[1] * offset_m, 0.0],
                dtype=np.float32,
            )
        )
