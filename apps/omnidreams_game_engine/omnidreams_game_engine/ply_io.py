...
"""Numpy-only PLY I/O for triangle meshes (vertices + faces).
...
:mod:`omnidreams_game_engine.simulation.ground_snap`) and to emit a flat
synthetic ground mesh from :mod:`omnidreams_game_engine.scene_fixture`.
...
def _parse_header(stream: io.BytesIO) -> tuple[_Format, list[_Element]]:
    ...
    while True:
        ...
        if tokens[0] == "end_header":
            ...
            if tokens[1] not in valid:
                raise ValueError(
                    f"Unsupported PLY format: {tokens[1]!r} (expected one of {sorted(valid)})"
                )
...
def _find_elements(elements: list[_Element]) -> tuple[_Element, _Element]:
...
def _load_ascii(
    ...
    for elem in elements:
        if elem is vertex_elem:
            ...
            vertices = np.empty((elem["count"], 3), dtype=np.float32)
...
def load_mesh_vf(data: bytes) -> tuple[np.ndarray, np.ndarray]:
    ...
    Args:
        data: Complete PLY file contents.

    Returns:
        ``(vertices, faces)`` where *vertices* is ``(N, 3)`` float32 and
        *faces* is ``(M, 3)`` int32 of triangle vertex indices.
    """
